...
class TestRankingConfig:
    ...
    def test_weights_validation(self) -> None:
        with pytest.raises(ValueError, match="sum to 1.0"):
            RankingConfig(w_rtt=0.5, w_loss=0.5, w_jitter=0.5, w_success=0.5, w_history=0.5)
...
class TestRankingEngine:
    ...
    def test_rank_basic(self, engine: RankingEngine) -> None:
        servers = [
            ServerScore("10.0.0.1", avg_rtt=10.0, loss_rate=0.01, jitter=1.0,
                        success_rate=0.99, samples=500),
            ServerScore("10.0.0.2", avg_rtt=50.0, loss_rate=0.10, jitter=10.0,
                        success_rate=0.90, samples=100),
        ]
    ...
    def test_rank_ordering(self, engine: RankingEngine) -> None:
        servers = [
            ServerScore("bad", avg_rtt=100.0, loss_rate=0.3, jitter=20.0,
                        success_rate=0.7, samples=10),
            ServerScore("good", avg_rtt=15.0, loss_rate=0.01, jitter=2.0,
                        success_rate=0.99, samples=500),
            ServerScore("mid", avg_rtt=40.0, loss_rate=0.05, jitter=5.0,
                        success_rate=0.95, samples=200),
        ]
        ranked = engine.rank(servers)
    ...
    def test_top_n(self, engine: RankingEngine) -> None:
        servers = [
            ServerScore(f"srv{i}", avg_rtt=float(i * 10),
                        loss_rate=0.01, success_rate=0.99, samples=100)
            for i in range(10)
        ]
    ...
    def test_get_best(self, engine: RankingEngine) -> None:
        servers = [
            ServerScore("a", avg_rtt=10.0, loss_rate=0.01, success_rate=0.99, samples=100),
            ServerScore("b", avg_rtt=50.0, loss_rate=0.10, success_rate=0.80, samples=100),
        ]
    ...
    def test_score_server(self, engine: RankingEngine) -> None:
        server = ServerScore("test", avg_rtt=15.0, loss_rate=0.02,
                             success_rate=0.98, samples=200)
    ...
    def test_compare(self, engine: RankingEngine) -> None:
        a = ServerScore("a", avg_rtt=10.0, loss_rate=0.01, success_rate=0.99, samples=200)
        b = ServerScore("b", avg_rtt=50.0, loss_rate=0.10, success_rate=0.85, samples=200)
        ra, rb, winner = engine.compare(a, b)
        assert winner.host == "a"

    def test_explain(self, engine: RankingEngine) -> None:
        server = ServerScore("test", avg_rtt=15.0, loss_rate=0.02,
                             jitter=3.0, success_rate=0.98, samples=200)
        ...
        contribs = sum(
            c["contribution"] for c in explanation["components"].values()
        )
    ...
    def test_confidence_affects_ranking(self, engine: RankingEngine) -> None:
        ...
        a = ServerScore("confident", avg_rtt=15.0, loss_rate=0.02,
                        success_rate=0.98, samples=1000, cv=0.05)
        b = ServerScore("uncertain", avg_rtt=15.0, loss_rate=0.02,
                        success_rate=0.98, samples=5, cv=0.3)
    ...
    def test_history_affects_ranking(self, engine: RankingEngine) -> None:
        ...
        a = ServerScore("stable", avg_rtt=20.0, loss_rate=0.05,
                        success_rate=0.95, samples=100,
                        history=[(0, True), (1, True), (3, True), (7, True)])
        b = ServerScore("unstable", avg_rtt=20.0, loss_rate=0.05,
                        success_rate=0.95, samples=100,
    ...
    def test_scores_in_range(self, engine: RankingEngine) -> None:
        servers = [
            ServerScore("a", avg_rtt=15.0, loss_rate=0.02, jitter=3.0,
                        success_rate=0.98, samples=200),
        ]
        ranked = engine.rank(servers)
        s = ranked[0]
    ...
    def test_equal_servers(self, engine: RankingEngine) -> None:
        ...
        s1 = ServerScore("a", avg_rtt=20.0, loss_rate=0.05, success_rate=0.95, samples=100)
        s2 = ServerScore("b", avg_rtt=20.0, loss_rate=0.05, success_rate=0.95, samples=100)
    ...
    def test_custom_config(self) -> None:
        ...
        cfg = RankingConfig(w_rtt=0.70, w_loss=0.10, w_jitter=0.05, w_success=0.05, w_history=0.10)
        engine = RankingEngine(config=cfg)
        servers = [
            ServerScore("fast", avg_rtt=5.0, loss_rate=0.10, success_rate=0.90, samples=100),
            ServerScore("reliable", avg_rtt=50.0, loss_rate=0.01, success_rate=0.99, samples=100),
        ]
        ranked = engine.rank(servers)
